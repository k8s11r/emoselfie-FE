...
| 2 | 상태 모델·시계·복구 기반 | roomStore는 LobbySnapshot 중심이고 RoomPage는 복원 시 Lobby를 렌더한다. serverClock은 계산 함수만 있다 | FE-082~084·120~122: screen별 복원 및 snapshot/event 경합 검증 |
| 3 | 대기실 → 촬영 → 서버 접수 | CaptureStage는 구현되어 있으나 RoomPage 게임 경로 연결이 필요하다 | FE-162~163·073~074·085~087: 두 명이 시작·촬영·202 인정까지 진행 |
| 4 | 결과 → 다음 라운드 → 최종 | 캐러셀·레일·리액션·스킵과 미제출 경로 구현 | FE-088~093·100~116: 제출자/미제출자 흐름을 함께 완주 |
