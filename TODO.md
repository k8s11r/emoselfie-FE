...
### 2026-09-08 결과 화면 점수 표시 점검

시안 재작업 과정에서 `spec.md` §4.8 항목 몇 개가 빠졌던 것을 확인하고 되돌렸다. 점수 숫자 형식(소수 한 자리, failed를 0으로 변환하지 않음)은 유지되고 있었다.

- 순위 포인트(SC-09)가 재작업 때 누락됐다. 확정 후에만 `+100 포인트`로 점수와 단위를 구분해 다시 표시한다. `FE-104` 완료 기록은 이 회귀를 고친 시점 기준으로 유효하다.
- 레일 순위 배지가 서버 값이 없을 때 FE가 계산한 위치(`index + 1`)를 보여 주고 있었다. 시안대로 `·`로 바꿔 순위를 지어내지 않는다.
- 내 제출이 아직 채점되지 않았을 때 `—` 대신 `판별 중`을 표시한다.
- 확정 후에도 `n / m 채점 완료`에 머물 수 있던 채점 현황을 `n명 결과 확정`으로 전환한다.
- 확정 결과의 `targetScore`·`rank`가 null이면 잠정값으로 되돌아가던 fallback을 제거했다. 서버가 null을 주면 null로 표시한다.
- 리액션 접근성 이름을 `좋아요`·`판정이 궁금해요`로 바꿨다. 명세가 금지한 시안 문구(`에계`, `이게 1등이라고?`)를 제거했다.
- 검증: `pnpm test` 21개 파일·99개 테스트, `pnpm lint`, `pnpm build` 통과. `src/features/game/GameScreens.test.tsx`에 회귀 테스트 4건 추가. 터널 경유 `/__preview/game`에서 눈으로 확인.

### 2026-09-08 모바일 촬영 영역 누락 수정

- 사용자 보고: iPhone Safari에서 촬영 영역 자체가 보이지 않음.
- 재현: 높이 720px 이하 CSS에서 `.capture-frame`에 width:auto와 자동 좌우 여백이 적용되고, 내부 video/review가 absolute라 크기 계산에 기여하지 않아 390×667 뷰포트에서 프레임이 8×10.66px로 축소됨.
- 수정: flex 축소를 방지하고, 짧은 화면에서도 명시적인 폭을 계산해 3:4 비율을 유지. 매우 낮은 가로 화면에서는 최소 폭 180px를 유지하고 세로 스크롤을 허용.
- 브라우저 레이아웃 검증: 390×667→321.75×429px, 320×568→247.5×330px, 390×844→336×448px, 844×390→180×240px. 가로 넘침 없음. 실제 iPhone 센서/Safari 엔진에서 영상 재생을 확인한 기록은 아님.
- `/__preview/game`에 카메라 권한을 요청하지 않는 촬영 레이아웃 예시 추가. 운영 빌드에서는 제외됨.
- 검증: lint, 테스트 21개 파일·99개 테스트, TypeScript 포함 build, diff whitespace 검사 통과.
